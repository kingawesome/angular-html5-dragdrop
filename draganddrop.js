...
angular.module("ngDragDrop", [])
  .directive("uiDraggable", [
    ...
    '$document',
    '$timeout',
    function ($parse, $rootScope, $document, $timeout) {
      return function (scope, element, attrs) {
        ...
        element.bind("dragstart", function (e) {
          ...
          if (attrs.dragHelper) {
            var el = angular.element($parse(attrs.dragHelper)(scope))
            var coords = $parse(attrs.dragHelperCoords)(scope) || {x: 0, y: 0}
            $document.find('body').append(el)
            e.dataTransfer.setDragImage(el.get(0), coords.x, coords.y);
            $timeout(function () {
              el.remove();
            }, 0)
          }
//          var el = angular.element('<div class="column-drag-helper">ASSIGNED</div>')
